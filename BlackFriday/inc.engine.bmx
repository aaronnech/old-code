...
'Clock, the player has to race against
Type TClock
	Field time:Int = 30
	Field going:Byte
	Field timetracker:Int
	
	Method Reset()
		time = 30
		timetracker = MilliSecs()
	End Method
	
	Method Update()
		If going
			If timetracker + 1000 <= MilliSecs() time:-1 timetracker = MilliSecs()
			If time = 0 RanOut()
		Else
			If level =.5 going = True Reset()
		End If
		Display()
	End Method
	...
	Method Display()
		SetImageFont(analog)
		DrawImage stopwatch, 859, 5
		SetColor 0, 255, 0
		If time < 10
			DrawText "0" + time, 939 - (TextWidth("0" + time) / 2), 55
		Else
			DrawText time, 939 - (TextWidth(time) / 2), 55
		End If
...
Type TPlayer Extends TEntity
	...
	Method Update()
		...
		If KeyDown(KEY_UP) And Acceleration < 12
			Acceleration:+.25 hasmoved = True
		ElseIf Acceleration > 0 And Not KeyDown(KEY_UP)
			Acceleration:-.25
		End If
...
Type TTutorial
	...
	Method CheckLevel()
		...
		If level =.25
			...
			New TPopup.Create("&Press [SPACE] to throw!", 750, 100)
		End If
		If level =.5
			New TPopup.Create("Good Luck!", 0, 0)
			New TPopup.Create("You are timed.", 500, 0)
...
'Popup used by the tutorial object
...
'Random Facts
Type TFact Extends TEntity
	Field cfact:Int'current fact
	Field show:Byte'Show the fact or not?
	Field facts:String[19]'19 random facts
	
	Method New()
		x = 270
		y = 650
		facts[0] = "Bad economies lure more shoppers since there's more competition for good deals."
		facts[1] = "In 2008, a Walmart employee was killed getting trampled."
		facts[2] = "The term 'Black Friday' has been traced back only To the 1960s."
		facts[3] = "Originally, 9-24-1869 was deemed Black Friday; a day of stock market catastrophe."
		facts[4] = "2009: MP3 Players drop from $132 to $88, a 33% decrease"
		facts[5] = "2009: GPS systems drop from $208 to $167, a 20% decrease"
		facts[6] = "2009: Microwave Ovens drop from $289 to $256, a 12% decrease"
		facts[7] = "2009: Docking Stations drop from $138 to $69, a 50% decrease"
		facts[8] = "2009: Plasma & LCD TVs drop from $1296 to $963, a 26% decrease"
		facts[9] = "The most popular item in 2009: Wii Console"
		facts[10] = "The third most popular item in 2009: Apple iPod touch 8GB"
		facts[11] = "The seventh most popular item in 2009: Wii Fit"
		facts[12] = "The eighth most popular item in 2009: Apple iPod touch 16GB"
		facts[13] = "Surveys say that 45 percent of consumers were planning to shop on Black Friday."
		facts[14] = "Cyber Monday is the name for the Monday following Black Friday."
		facts[15] = "Surveys say that 66 percent of consumers were planning to shop on Cyber Monday."
		facts[16] = "Spending in 2008 reached an estimated $41 Billion in the US."
		facts[17] = "Some websites offer information about Black Friday specials up to a month in advance."
		facts[18] = "The term Cyber Thanksgiving, refers to online retailer's Thanksgiving Day promotions."
	End Method
	...
	Method Update()
		If show
			SetColor 0,0,0
			DrawRect x-4,y-4,748,108
			SetColor 255,255,255
			DrawRect x,y,740,100
			SetColor 0,0,0
			DrawText "Black Friday Fact:",x+20,y+20
			DrawText facts[cfact],x+370-(TextWidth(facts[cfact])/2),y+50-(TextHeight(facts[cfact])/2)
			SetColor 255,255,255
			If GIFT = False
				show=False
			End If
...
Function DrawBG()
	...
	DrawImage snowballimg, 20, 735'Draw the snow ball status
	SetColor 230, 20, 20
		SetImageFont(pointfont)
		DrawText POINTS, 156, 690'Draw the gifts gotten
		DrawText "x " + player.Snowballs, 50, 725'Snow ball status
		SetImageFont(myfont)
	SetColor 255, 255, 255
End Function
...
Function GetBlurredSS:TImage()
	...
	px = ResizePixmap(px, 256, 192)
	Local myImage:TImage = LoadImage(px)
	
	
	Return myImage'return the blurred background screenshot
End Function
